...
voter_turnout_df1 <- read_excel("data/G04ResultsDetail2004-11-02.xls", sheet = "Voter Turnout",
                                range = NULL, col_names = TRUE,skip=1)
...
con <- dbConnect(drv = RSQLite::SQLite(),dname = "data/tidynomicon/example.db")
con <- dbConnect(RSQLite::SQLite(), "data/tidynomicon/example.db")
